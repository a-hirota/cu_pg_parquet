...
from src.types import PG_OID_TO_ARROW, UNKNOWN, ColumnMeta
...
def setup_rmm_pool():
    ...
    try:
        ...
        rmm.reinitialize(
            pool_allocator=True, initial_pool_size=pool_size, maximum_pool_size=pool_size
        )
    ...
    for chunk_id in range(total_chunks):
        ...
        try:
            ...
            if os.environ.get("GPUPGPARSER_TEST_MODE") == "1" and chunk_id == 0:
                ...
                print(
                    f"[Producer] Rust stderr: {process.stderr}"
                    if process.stderr
                    else "[Producer] stderrは空"
                )
            ...
            if (
                chunk_id == 0
                and json_start == -1
                and os.environ.get("GPUPGPARSER_TEST_MODE") == "1"
            ):
            ...
            print(
                f"[Producer] チャンク {chunk_id + 1} 転送完了 ({rust_time:.1f}秒, {file_size / 1024**3:.1f}GB)"
            )
...
def gpu_consumer(
    ...
    while not shutdown_flag.is_set():
        try:
            ...
            print(
                f"[Consumer-{consumer_id}] チャンク {chunk_id + 1} GPU処理完了 ({gpu_time:.1f}秒, {rows:,}行)"
            )
...
def run_parallel_pipeline(
    total_chunks: int, table_name: str, test_mode: bool = False, compression: str = "zstd"
):
    """真の並列パイプライン実行"""
    # キューとスレッド管理
    ...
    warmup_thread = threading.Thread(target=gpu_warmup, args=(columns,))
    warmup_thread.start()
...
def gpu_warmup(columns):
    ...
    try:
        ...
        # 実際の処理と同じ環境変数を設定
        os.environ["GPUPGPARSER_ROWS_PER_THREAD"] = os.environ.get(
            "GPUPGPARSER_ROWS_PER_THREAD", "32"
        )
        os.environ["GPUPGPARSER_STRING_ROWS_PER_THREAD"] = os.environ.get(
            "GPUPGPARSER_STRING_ROWS_PER_THREAD", "1"
        )
...
def main(
    total_chunks=8, table_name=None, test_mode=False, test_duplicate_keys=None, compression="zstd"
):
    global TABLE_NAME
    if table_name:
        TABLE_NAME = table_name
    ...
    try:
        ...
        if results["total_rust_time"] > 0:
            print(
                f"├─ Rust転送合計: {results['total_rust_time']:.2f}秒 ({total_gb / results['total_rust_time']:.2f} GB/秒)"
            )
        else:
            print(f"├─ Rust転送合計: {results['total_rust_time']:.2f}秒")
        if results["total_gpu_time"] > 0:
            print(
                f"└─ GPU処理合計: {results['total_gpu_time']:.2f}秒 ({total_gb / results['total_gpu_time']:.2f} GB/秒)"
            )
        else:
        ...
        if chunk_stats:
            ...
            for stat in sorted_stats:
                print(
                    f"│  {stat['chunk_id']:^3}  │ {stat['rust_time']:>6.2f}秒 │ {stat['transfer_time']:>6.2f}秒 │"
