...
use tokio_postgres::{NoTls, Config, CopyOutStream};
...
const PARALLEL_CONNECTIONS: usize = 32;  // 並列接続数を増やす
const BUFFER_SIZE: usize = 16 * 1024 * 1024;  // 16MBバッファ
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("=== PostgreSQL 最適化並列COPY BINARY ベンチマーク ===");
    ...
    for task_idx in 0..PARALLEL_CONNECTIONS {
        let start_page = task_idx as u32 * pages_per_task;
        let end_page = if task_idx == PARALLEL_CONNECTIONS - 1 {
            max_page + 1
        } else {
    ...
    if speed_gbps < 7.0 {
        ...
        println!("\n追加の改善案:");
        println!("- PostgreSQLの設定調整（work_mem, shared_buffers）");
        println!("- ネットワークMTUサイズの最適化");
        println!("- カーネルパラメータの調整（tcp_rmem, tcp_wmem）");
        println!("- NVMe SSDの並列I/O最適化");
    } else {
        println!("\n✓ 目標達成！");
    }
...
async fn process_range_optimized(
    config: Config,
    start_page: u32,
    end_page: u32,
    task_idx: usize,
    ...
    while let Some(chunk) = stream.next().await {
        ...
        // 共有カウンタを更新（アトミック操作の頻度を下げる）
        if task_chunks % 100 == 0 {
            total_bytes.fetch_add(task_bytes, Ordering::Relaxed);
            total_chunks.fetch_add(task_chunks, Ordering::Relaxed);
            task_bytes = 0;
            task_chunks = 0;
        }
    }
