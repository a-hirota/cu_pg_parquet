...
詳細はCLAUDE.mdまたはREADME.mdを参照してください。
    ...
    parser.add_argument("--chunks", type=int, default=8, help="チャンク数（デフォルト: 8）")
    parser.add_argument(
        "--output-dir", type=str, default="./output", help="出力ディレクトリ（デフォルト: ./output）"
    )
    parser.add_argument("--test", action="store_true", help="テストモード（Grid境界スレッド情報を出力）")
    parser.add_argument("-y", "--yes", action="store_true", help="既存のParquetファイルを確認なしで削除")
    parser.add_argument("--output", type=str, help="出力ディレクトリ（--output-dirのエイリアス）")
    parser.add_argument(
        "--test-duplicate-keys",
        type=str,
        default=None,
