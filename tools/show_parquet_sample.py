...
def check_gpu_environment():
    ...
    try:
        ...
        for i in range(device_count):
            ...
            print(f"  GPU {i}: {name}")
            print(
                f"    メモリ: {mem_info.total / 1024**3:.1f} GB (使用中: {mem_info.used / 1024**3:.1f} GB)"
            )
                ...
                if not isinstance(df, dask_cudf.DataFrame):
                    try:
                        ...
                        if len(unique_values) <= 20:
                            ...
                            # 数値型の場合は最小値と最大値を表示
                            if str(df[filter_column].dtype) in [
                                "int8",
                                "int16",
                                "int32",
                                "int64",
...
def process_large_parquet_files(
    filter_column=None, filter_value=None, target_dir=None, sort_column=None, file_path=None
):
    """メイン処理フロー"""
    ...
    if sort_column and len(dfs) > 1:
        ...
        for i, (df, file_path) in enumerate(zip(dfs, file_paths)):
            display_sample_data(
                df,
                n_rows=5,
                name=f"ファイル{i} ({Path(file_path).name})",
                filter_column=filter_column,
                filter_value=filter_value,
                file_path=file_path,
                sort_column=sort_column,
...
使用例:
  ...
  python show_parquet_sample.py --file output/customer_chunk_0_queue.parquet --filter c_custkey=12345
    ...
    parser.add_argument(
        "--thread_id", type=int, help="フィルタリングするthread_id（非推奨: --filter _thread_id=値 を使用してください）"
    )

    parser.add_argument("--filter", type=str, help="フィルタリング条件（形式: カラム名=値）")

    parser.add_argument(
        "--dir", type=str, help="Parquetファイルが格納されているディレクトリ（デフォルト: outputまたは現在のディレクトリ）"
    )

    parser.add_argument(
        "--sort", type=str, help="ソートするカラム名（整数型・decimal型対応）。複数ファイルは自動的に結合され、欠落値分析も実行されます"
    )

    parser.add_argument("--file", type=str, help="読み込む特定のParquetファイルパス")
