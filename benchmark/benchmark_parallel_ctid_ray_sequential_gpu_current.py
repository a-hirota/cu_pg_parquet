...
OUTPUT_PARQUET_PATH = "benchmark/lineorder_parallel_ctid_ray_sequential_gpu_current.output"
...
class PostgreSQLWorker:
    ...
    def copy_data_to_memory(
        ...
        """PostgreSQLからデータをCOPYしてピンメモリに保存（現在の実装）"""
        
        # メトリクス初期化
        metrics = DetailedMetrics(is_worker=True)  # Ray Worker内なのでTrue
        process = psutil.Process()
        initial_memory = process.memory_info().rss / (1024**2)  # MB
        initial_memory_gb = initial_memory / 1024  # 既存ログ用
        ...
        print(f"  初期メモリ使用量: {initial_memory_gb:.2f} GB")
        ...
        try:
            ...
            try:
                ...
                with conn.cursor() as cur:
                    with cur.copy(copy_sql) as copy_obj:
                        if batch_read:
                            ...
                            # メモリ使用量（BytesIO書き込み後）
                            metrics.log_memory_snapshot("after_bytesio")
                            memory_after_io = process.memory_info().rss / (1024**3)
                            print(f"  チャンク読み込み完了: {chunk_count}個, 合計{chunk_total_size/(1024**2):.2f} MB")
                            print(f"  メモリ使用量（BytesIO書き込み後）: {memory_after_io:.2f} GB (+{memory_after_io - initial_memory_gb:.2f} GB)")
                            
                            # BytesIOからbytearrayを取得
                            buffer = bytearray(buffer_io.getvalue())
                            
                            # メモリ使用量（最終）
                            metrics.log_memory_snapshot("after_bytearray")
                ...
                # メトリクス記録
                metrics.log_memory_snapshot("after_copy")
                ...
                metrics.metrics['memory']['total_data_size_mb'] = data_size / (1024**2)
                ...
                if data_size > 0:
                    overhead_ratio = memory_used / (data_size / (1024**2))
                else:
                    overhead_ratio = 1.0
                ...
                print(f"  最終メモリ使用量: {final_memory:.2f} GB (合計増加: +{final_memory - initial_memory_gb:.2f} GB)")
...
def run_ray_parallel_sequential_gpu(
    ...
    print(f"=== PostgreSQL → GPU Ray並列 順次GPU処理版（現在の実装 + メトリクス） ===")
    ...
    print(f"COPY読み込み方式: {'一括読み込み最適化（BytesIO + bytearray）' if copy_buffer_size > 0 else '従来方式（逐次追加）'}")
    ...
    if not ray.is_initialized():
        ray.init(num_cpus=parallel_count * 2)  # GPUはメインプロセスのみ
        print(f"✅ Ray初期化完了（CPU並列処理）")
    ...
    if all_worker_metrics:
        ...
        global_metrics.metrics['performance']['gpu_transfer_count'] = len(gpu_results)
    ...
    print(f"\n{'='*60}")
    print(f"=== Ray並列セミパイプラインGPU処理ベンチマーク完了（現在の実装） ===")
    print(f"{'='*60}")
    ...
    print("\n--- 処理方式の特徴 ---")
    print("  ✅ 現在の実装: BytesIO + bytearray（中間バッファあり）")
    ...
    # メトリクスをJSONファイルに保存
    metrics_file = f"{OUTPUT_PARQUET_PATH}_metrics_current.json"
