"""
PostgreSQL → GPU 究極最適化版 (修正版)
転送速度を10GB/s近くまで引き上げる

ユーザー分析に基づく最適化:
- libpqチャンクを蓄積して大きなブロックで処理
- pinned memcpyの最適化
- 64MiB以上の大きなチャンクサイズ
- 複数ストリームでの非同期DMA
...
def process_chunk_to_gpu(chunk, offset, devbuf, current_buf, current_stream, total_memcpy_time, total_async_time):
    """チャンクをGPUに転送する処理"""
    ...
    if offset + chunk_size > devbuf.size:
        ...
        return False, total_memcpy_time, total_async_time
    ...
    return True, total_memcpy_time, total_async_time
...
def run_ultimate_optimized_benchmark(limit_rows=1000000):
    ...
    print(f"  蓄積目標サイズ: {READ_SIZE / (1024*1024):.0f} MB")
    ...
    try:
            ...
            with cur.copy(copy_sql) as copy_obj:
                ...
                for chunk in copy_obj:
                    ...
                    if accumulated_size >= READ_SIZE:
                        ...
                        # ダブルバッファ選択
                        current_buf = pbuf1 if block_count % 2 == 0 else pbuf2
                        current_stream = streams[block_count % NUM_STREAMS]
                        
                        # GPU転送処理
                        success, total_memcpy_time, total_async_time = process_chunk_to_gpu(
                            combined_chunk, offset, devbuf, current_buf, current_stream,
                            total_memcpy_time, total_async_time
                        )
                        
                        if not success:
                            break
                        
                        offset += len(combined_chunk)
                        block_count += 1
                        ...
                        # 進捗表示
                        if block_count % 50 == 0:
                            avg_block_size = len(combined_chunk) / (1024*1024)
                            print(f"    ブロック {block_count:,} | {offset / (1024*1024):.0f} MB | 平均 {avg_block_size:.1f} MB/block")
        ...
        print(f"  処理ブロック数: {block_count:,}")
        if block_count > 0:
            print(f"  平均ブロックサイズ: {(actual_data_size / block_count) / (1024*1024):.2f} MB")
    ...
    # PCIe効率計算（実測11.7GB/s基準）
    pcie_efficiency = network_throughput / 11700 * 100
    print(f"  PCIe効率        : {pcie_efficiency:.1f}% (対11.7GB/s実測)")
    
    print("--- 究極最適化効果 ---")
    print("  ✅ ファイル I/O: 完全ゼロ")
    print("  ✅ チャンク蓄積: 小さなチャンクを4MBブロックに集約")
...
def main():
    """メイン関数"""
    parser = argparse.ArgumentParser(description='PostgreSQL → GPU 究極最適化版 (修正版)')
    ...
    parser.add_argument('--read-size', type=int, default=4, help='蓄積目標サイズ(MB)')
