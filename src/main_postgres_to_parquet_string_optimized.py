...
統合バッファは維持し、文字列処理のみを最適化:
1. 文字列データ: 共有メモリ不使用の直接コピー
2. 固定長データ: 既存の統合カーネルを使用（変更なし）
...
class StringOptimizedProcessor:
    """文字列処理最適化プロセッサー"""
    ...
    def create_optimized_string_buffers(
        ...
        for col_idx, col in enumerate(var_columns):
            ...
            try:
                ...
                warnings.warn(f"最適化文字列バッファ作成エラー ({col.name}): {e}")
                string_buffers[col.name] = {'data': None, 'offsets': None, 'actual_size': 0}
        
        return string_buffers
    
    def decode_and_export_with_string_optimization(
        self,
        ...
        **parquet_kwargs
        ...
        # 最適化文字列バッファ作成（共有メモリ不使用）
        optimized_string_buffers = self.create_optimized_string_buffers(
    ...
    def process_postgresql_to_parquet_string_optimized(
        self,
        raw_dev: cuda.cudadrv.devicearray.DeviceNDArray,
        columns: List[ColumnMeta],
        ncols: int,
        ...
        **kwargs
        ...
        print("=== 文字列最適化デコード開始 ===")
        cudf_df, decode_timing = self.decode_and_export_with_string_optimization(
            raw_dev, field_offsets_dev, field_lengths_dev,
...
def postgresql_to_cudf_parquet_string_optimized(
    raw_dev: cuda.cudadrv.devicearray.DeviceNDArray,
    columns: List[ColumnMeta],
    ncols: int,
    header_size: int,
    ...
    **parquet_kwargs
    ...
    """
    PostgreSQL → cuDF → GPU Parquet 文字列最適化処理関数
    
    文字列処理のみを最適化した安全なアプローチ:
    - 文字列データ: 共有メモリ不使用の直接コピー
    - 固定長データ: 既存の統合カーネルを使用（変更なし）
    - cuDFによるゼロコピーArrow変換
    - GPU直接Parquet書き出し
    - RMM統合メモリ管理
    ...
    processor = StringOptimizedProcessor(
        use_rmm=use_rmm, 
        optimize_gpu=optimize_gpu
    )
    
    return processor.process_postgresql_to_parquet_string_optimized(
...
__all__ = [
    "StringOptimizedProcessor",
    "postgresql_to_cudf_parquet_string_optimized"
]