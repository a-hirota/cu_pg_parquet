...
from numba import cuda, int64, int32, uint32, uint64, uint16, int8, uint8, float32, float64
import cupy as cp
...
def parse_with_range_debug(raw_dev, columns, header_size=None, mb_start=128, mb_end=549):
    ...
    parse_rows_and_fields_lite_with_range_debug[blocks, threads_per_block](
        ...
        row_count, thread_stride, max_rows, fixed_field_lengths,
    ...
    if debug_count_host > 0:
        ...
        for i in range(min(20, debug_count_host)):  # 最初の20個
            ...
            print(f"  Thread {tid}: 0x{start:08X}-0x{end:08X} ({start/(1024*1024):.1f}MB-{end/(1024*1024):.1f}MB), {rows}行検出")
