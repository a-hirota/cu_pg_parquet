...
def extract_fields_coalesced_optimized(
    ...
    if field_start_pos + 3 < raw_data.size:
        field_len = ((raw_data[field_start_pos] << 24) |
    ...
    for col_idx in range(num_cols):
        ...
        if not is_variable:
            ...
            for i in range(copy_size):
                if (field_offset + i < raw_data.size and 
                    output_offset + i < fixed_outputs[col_idx].size):
            ...
            if var_idx >= 0:
                ...
                for i in range(field_length):
                    if (field_offset + i < raw_data.size and 
                        current_offset + i < var_data_buffer.size):
...
def parse_binary_chunk_gpu_enhanced(
    ...
    if data_size <= 0:
        return cuda.device_array((0, ncols), dtype=np.int32), cuda.device_array((0, ncols), dtype=np.int32)
