...
import numpy as np
from numba import cuda, int32, int64, types, uint32, uint64
...
def compact_valid_rows(
    row_positions, field_offsets, field_lengths, out_offsets, out_lengths, invalid_value, n, ncols
):
    """有効な行のみを抽出して出力配列に格納"""
    idx = cuda.grid(1)
    if idx < n:
...
def estimate_row_size_from_columns(columns):
    """ColumnMetaから行サイズ推定"""
    try:
        from ..types import DECIMAL128, UTF8
    except ImportError:
        from src.types import DECIMAL128, UTF8
    ...
    for col in columns:
        ...
        if col.elem_size > 0:  # 固定長
            ...
            if debug:
                print(
                    f"[ESTIMATE DEBUG] {col.name}: 4 + {col.elem_size} = {4 + col.elem_size}バイト (固定長)"
                )
        else:  # 可変長の推定
            if col.arrow_id == UTF8:
                ...
                if col.arrow_param is not None and isinstance(col.arrow_param, int):
                    ...
                    if debug:
                        print(
                            f"[ESTIMATE DEBUG] {col.name}: 4 + {col.arrow_param} = {4 + col.arrow_param}バイト (bpchar)"
                        )
                else:
...
def print_gpu_properties():
    ...
    print(
        f"Compute Capability:       {props.get('COMPUTE_CAPABILITY', (0,0))[0]}.{props.get('COMPUTE_CAPABILITY', (0,0))[1]}"
    )
    ...
    print(
        f"Max Grid Dimensions:     X={props.get('MAX_GRID_DIM_X', 0):,}, Y={props.get('MAX_GRID_DIM_Y', 0):,}"
    )
...
def validate_and_extract_fields_lite(
    ...
    for field_idx in range(num_fields):
        ...
        if flen == 0xFFFFFFFF:  # NULL
            ...
            if (
                field_idx < expected_cols
                and fixed_field_lengths[field_idx] > 0
                and flen != fixed_field_lengths[field_idx]
            ):
                return False, -5
            ...
            if field_idx < expected_cols and column_pg_oids[field_idx] == 1700 and flen >= 8:
                # numeric型のヘッダー検証
...
@cuda.jit
def analyze_negative_positions(row_positions, nrows, raw_data, negative_debug, negative_count):
    """負の行位置を分析してデバッグ情報を記録"""
...
def parse_postgres_raw_binary_to_column_arrows(
    raw_dev, columns, header_size=None, debug=False, test_mode=False
):
    """
    ...
    if debug or test_mode:
        ...
        if variable_fields:
            print(
                f"{prefix} [DEBUG] 可変長フィールド: {', '.join(variable_fields[:3])}{'...' if len(variable_fields) > 3 else ''}"
            )
        if decimal_fields:
    ...
    if test_mode:
        ...
        prefix = f"[Consumer-1]" if chunk_id >= 0 else ""
        print(
            f"{prefix} [GRID計算] target_threads={target_threads:,}, blocks_x={blocks_x:,}, blocks_y計算値={blocks_y:,}"
        )
        if rows_per_thread > 1:
            print(
                f"{prefix} [GRID計算] rows_per_thread={rows_per_thread}, optimal_threads={optimal_threads:,}, blocks_per_sm={blocks_per_sm}"
            )
    ...
    if thread_stride < estimated_row_size:
        ...
        # 行サイズの倍数に切り上げ
        thread_stride = (
            (thread_stride + estimated_row_size - 1) // estimated_row_size
        ) * estimated_row_size
    ...
    if rows_per_thread > 200:
        if debug or test_mode:
            ...
            print(
                f"{prefix}            thread_stride={thread_stride:,} バイト, estimated_row_size={estimated_row_size} バイト"
            )
    ...
    if debug or test_mode:
        ...
        prefix = f"[Consumer-1]" if chunk_id >= 0 else ""
        print(
            f"{prefix} [DEBUG] estimated_rows計算: data_size={data_size:,}, estimated_row_size={estimated_row_size}, estimated_rows={estimated_rows:,}"
        )
    # GPUメモリ上限の確認
    bytes_per_row = 8 + ncols * (
        4 + 4
    )  # row_positions(8) + field_offsets(4*ncols) + field_lengths(4*ncols)
    ...
    if debug or test_mode:
        ...
        print(
            f"{prefix} [DEBUG] GPU行位置情報配列: 1行あたり{bytes_per_row}バイト (row_pos:8 + offsets:{offsets_size} + lengths:{lengths_size})"
        )
        print(
            f"{prefix} [DEBUG] 推定必要メモリ: {bytes_per_row}バイト × {estimated_rows:,}行 = {estimated_memory_mb:.0f}MB"
        )
        print(
            f"{prefix} [DEBUG] 実際の配列確保: {max_rows:,}行分 = {actual_memory_mb:.0f}MB (推定の{(max_rows/estimated_rows*100):.1f}%)"
        )
    ...
    if debug:
        ...
        print(
            f"{prefix} [DEBUG] 推定総行数: {estimated_rows:,}, スレッドあたり: {thread_stride//estimated_row_size:.1f}行"
        )
    ...
    if test_mode:
        ...
        print(f"{prefix} " + "=" * 60)
        print(
            f"{prefix} Data Size:               {data_size:,} bytes ({data_size/1024/1024:.1f} MB)"
        )
        ...
        if rows_per_thread > 1:
            ...
            print(
                f"{prefix} Thread Reduction:        {estimated_rows:,} → {target_threads:,} ({(1-target_threads/estimated_rows)*100:.1f}% reduction)"
            )
        print(f"{prefix} " + "=" * 60 + "\n")

    if test_mode:
        # テストモードではデバッグ版カーネルを使用
    ...
    if debug or test_mode:
        ...
        if test_mode and thread_row_counts is not None and nrow > 0:
            ...
            print(
                f"{prefix}   - 行を検出したスレッド: {threads_with_data:,} ({threads_with_data/actual_threads*100:.1f}%)"
            )
            ...
            if max_rows_per_thread > 0:
                hist, bins = np.histogram(
                    thread_counts_host[thread_counts_host > 0], bins=min(10, max_rows_per_thread)
                )
                print(f"{prefix}   - 行数分布:")
            ...
            if thread_debug_info is not None:
                ...
                for i in range(min(10, actual_threads)):
                    ...
                    print(
                        f"{prefix}   スレッド{tid}: start_pos={start_pos:,}, end_pos={end_pos:,}, 検出行数={row_count}"
                    )
                ...
                print(
                    f"{prefix} [DEBUG] 最終スレッドのend_pos: {int(thread_debug_host[actual_threads-1, 2]):,}"
                )
    ...
    if nrow == 0:
        if test_mode:
            return (
                cuda.device_array(0, np.uint64),
                cuda.device_array((0, ncols), np.uint32),
                cuda.device_array((0, ncols), np.int32),
                cuda.device_array(0, np.int32),
                cuda.device_array(0, np.uint64),
                cuda.device_array(0, np.uint64),
            )
        return (
    ...
    if nrow > 0:
        ...
        if valid_rows > 0:
            ...
            if test_mode:
                ...
                final_thread_start_pos = (
                    thread_start_positions[:valid_rows]
                    if thread_start_positions is not None
                    else None
                )
                final_thread_end_pos = (
                    thread_end_positions[:valid_rows] if thread_end_positions is not None else None
                )
                return (
                    final_row_positions,
                    final_field_offsets,
...
# エイリアス関数（後方互換性）
def parse_binary_chunk_gpu_ultra_fast_v2(
    raw_dev, columns, header_size=None, debug=False, test_mode=False
):
    """従来版インターフェース互換ラッパー"""
    return parse_postgres_raw_binary_to_column_arrows(
        raw_dev, columns, header_size, debug, test_mode
    )


def parse_binary_chunk_gpu_ultra_fast_v2_integrated(
    raw_dev, columns, header_size=None, debug=False, test_mode=False
):
    """統合最適化版（軽量版を使用）"""
    return parse_postgres_raw_binary_to_column_arrows(
