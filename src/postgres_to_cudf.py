...
import rmm
from numba import cuda, int32, uint8, uint32

from .cuda_kernels.decimal_tables import POW10_TABLE_HI_HOST, POW10_TABLE_LO_HOST
...
class DirectColumnExtractor:
    ...
    def extract_columns_direct(
        ...
        if os.environ.get("GPUPGPARSER_TEST_MODE") == "1":
            ...
            string_cols = [
                c for c in columns if c.is_variable and (c.arrow_id == UTF8 or c.arrow_id == BINARY)
            ]
            ...
            if fixed_cols:
                ...
                type_name = (
                    first_fixed.arrow_id.name
                    if hasattr(first_fixed.arrow_id, "name")
                    else f"Type_{first_fixed.arrow_id}"
                )
    ...
    def _extract_fixed_column_direct(
        ...
        if col.arrow_id == DECIMAL128:
            ...
            col.arrow_id == INT64
            or col.arrow_id == FLOAT64
            or col.arrow_id == TS64_US
            or col.arrow_id == TS64_S
        ):
            col_size = 8
        ...
        column_cupy = cp.ndarray(
            ...
            memptr=cp.cuda.MemoryPointer(
                cp.cuda.UnownedMemory(
                    column_buffer_rmm.ptr, column_buffer_rmm.size, column_buffer_rmm
                ),
                0,
            ),
        )
        ...
        null_mask_cupy = cp.ndarray(
            ...
            dtype=cp.uint8,
            memptr=cp.cuda.MemoryPointer(
                cp.cuda.UnownedMemory(null_mask_rmm.ptr, null_mask_rmm.size, null_mask_rmm), 0
            ),
        )
        ...
        blocks_x = min(
            (total_threads_needed + threads_per_block - 1) // threads_per_block, max_blocks_x
        )
        blocks_y = (total_threads_needed + blocks_x * threads_per_block - 1) // (
        ...
        if os.environ.get("GPUPGPARSER_TEST_MODE") == "1" and col_idx == 0:
            ...
            data_type_name = (
                col.arrow_id.name if hasattr(col.arrow_id, "name") else f"Type_{col.arrow_id}"
            )
        ...
        if col.arrow_id == DECIMAL128:
            ...
            if os.environ.get("GPUPGPARSER_TEST_MODE") == "1" and columns:
                ...
                if is_first_decimal:
                    ...
                    print(
                        f"{prefix} Kernel Execution Info: _extract_decimal_direct (First Decimal)"
                    )
                    print(f"{prefix} ============================================================")
                    print(f"{prefix} Column: {col.name} (index: {col_idx})")
            ...
            self._extract_decimal_direct[grid, threads_per_block](
                ...
                column_buffer,
                null_mask,
                self.d_pow10_table_lo,
                self.d_pow10_table_hi,
                decimal_scale,
                rows,
                rows_per_thread,
            )
        ...
        return self._create_series_from_rmm_buffer(column_buffer_rmm, null_mask_rmm, col, rows)
            ...
            if is_null:
                ...
                # TS64_S=8 (PostgreSQL date型からの変換)
                    ...
                    if src_offset + 4 <= raw_data.size and dst_offset + 8 <= output_buffer.size:
                        ...
                        days_since_2000_unsigned = (
                            (raw_data[src_offset] << 24)
    ...
    def _create_series_from_buffer_fallback(
        self, column_buffer_rmm: rmm.DeviceBuffer, col: ColumnMeta, rows: int
    ) -> cudf.Series:
        ...
        try:
            ...
            data_cupy = cp.ndarray(
                ...
                memptr=cp.cuda.MemoryPointer(
                    cp.cuda.UnownedMemory(
                        column_buffer_rmm.ptr, rows * col_size, column_buffer_rmm
                    ),
                    0,
                ),
            )
    ...
    def _create_decimal_series_from_rmm(
        ...
        try:
            ...
            int64_cupy = cp.ndarray(
                ...
                dtype=cp.int64,
                memptr=cp.cuda.MemoryPointer(
                    cp.cuda.UnownedMemory(column_buffer_rmm.ptr, rows * 8, column_buffer_rmm), 0
                ),
            )
    ...
    def _create_string_series_from_buffer(
        self, col: ColumnMeta, rows: int, buffer_info: Dict[str, Any]
    ) -> cudf.Series:
        ...
        try:
            ...
            data_cupy = cp.asarray(
                cp.ndarray(
                    shape=(buffer_info["actual_size"],),
                    dtype=cp.uint8,
                    memptr=cp.cuda.MemoryPointer(
                        cp.cuda.UnownedMemory(data_ptr, buffer_info["actual_size"], data_buffer), 0
                    ),
                )
            )

            offsets_cupy = cp.asarray(
                cp.ndarray(
                    shape=(rows + 1,),
                    dtype=cp.int32,
                    memptr=cp.cuda.MemoryPointer(
                        cp.cuda.UnownedMemory(offsets_ptr, (rows + 1) * 4, offsets_buffer), 0
                    ),
                )
            )
            ...
            try:
                ...
                if isinstance(data_buffer, rmm.DeviceBuffer) and isinstance(
                    offsets_buffer, rmm.DeviceBuffer
                ):
                    ...
                    data_cupy = cp.asarray(
                        cp.ndarray(
                            ...
                            memptr=cp.cuda.MemoryPointer(
                                cp.cuda.UnownedMemory(
                                    data_ptr, buffer_info["actual_size"], data_buffer
                                ),
                                0,
                            ),
                        )
        ...
        for col_idx, col in enumerate(var_columns):
            ...
            try:
                ...
                lengths_cupy = cp.ndarray(
                    ...
                    memptr=cp.cuda.MemoryPointer(
                        cp.cuda.UnownedMemory(
                            lengths_buffer.ptr, lengths_buffer.size, lengths_buffer
                        ),
                        0,
                    ),
                )
                ...
                string_rows_per_thread = int(
                    os.environ.get("GPUPGPARSER_STRING_ROWS_PER_THREAD", "1")
                )
                ...
                if os.environ.get("GPUPGPARSER_TEST_MODE") == "1" and col_idx == 0:
                    ...
                    if string_rows_per_thread > 1:
                        ...
                        print(
                            f"{prefix} Thread Reduction: {rows:,} → {total_threads_needed:,} ({(1-total_threads_needed/rows)*100:.1f}% reduction)"
                        )
                    print(
                        f"{prefix} ============================================================\n"
                    )
                ...
                offsets_cupy = cp.ndarray(
                    ...
                    memptr=cp.cuda.MemoryPointer(
                        cp.cuda.UnownedMemory(
                            offsets_buffer.ptr, offsets_buffer.size, offsets_buffer
                        ),
                        0,
                    ),
                )
                ...
                data_cupy = cp.ndarray(
                    ...
                    dtype=cp.uint8,
                    memptr=cp.cuda.MemoryPointer(
                        cp.cuda.UnownedMemory(data_buffer.ptr, data_buffer.size, data_buffer), 0
                    ),
                )
                ...
                def copy_string_data_direct(
                    ...
                    for row in range(start_row, end_row):
                        ...
                        for i in range(field_length):
                            ...
                            # 境界チェック
                            if src_idx < raw_data.size and dst_idx < data_out.size:
