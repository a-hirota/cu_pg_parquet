...
class DirectProcessor:
    ...
    def process_direct(
        ...
        **parquet_kwargs,
        ...
        cudf_df = self.extractor.extract_columns_direct(
            ...
            thread_ids_dev,
            thread_start_positions_dev,
            thread_end_positions_dev,
        )
        ...
        parquet_timing = write_cudf_to_parquet_with_options(
            cudf_df, output_path, compression=compression, optimize_for_spark=True, **parquet_kwargs
        )
            ...
            for i in range(min(17, info[6])):
                if 7 + i * 2 + 1 < len(info):
                    debug_entry["field_info"].append(
                        {"offset": int(info[7 + i * 2]), "length": int(info[8 + i * 2])}
                    )
    ...
    def _print_negative_position_debug_info(self, negative_debug_info: np.ndarray, raw_dev):
        ...
        with tempfile.NamedTemporaryFile(
            mode="w", suffix="_negative_pos_debug.json", delete=False
        ) as f:
            json.dump(negative_data, f, indent=2)
            print(f"\n負の行位置デバッグ情報をJSONファイルに保存: {f.name}")

    def _print_performance_stats(
        self, rows: int, cols: int, timing: Dict[str, float], data_size: int
    ):
...
def convert_postgres_to_parquet_format(
    ...
    **parquet_kwargs,
    ...
    processor = DirectProcessor(
        use_rmm=use_rmm, optimize_gpu=optimize_gpu, verbose=verbose, test_mode=test_mode
    )

    return processor.transform_postgres_to_parquet_format(
        raw_dev, columns, ncols, header_size, output_path, compression, **parquet_kwargs
    )


__all__ = ["DirectProcessor", "convert_postgres_to_parquet_format"]
