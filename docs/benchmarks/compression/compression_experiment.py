...
def run_benchmark(
    table_name: str, parallel: int, chunks: int, compression: str, timeout: int = 300
):
    """単一の圧縮方式でベンチマークを実行"""
    print(f"\n{'='*80}")
...
def print_summary(results: list):
    ...
    # ヘッダー
    print(
        f"{'Compression':<12} {'Time (s)':<10} {'Size (MB)':<12} {'Ratio':<8} {'Throughput':<12} {'Status'}"
    )
    print("-" * 80)
    ...
    for r in results:
        ...
        print(
            f"{r['compression']:<12} {time_str:<10} {size_str:<12} {ratio_str:<8} {throughput_str:<12} {status}"
        )
    ...
    if successful_results:
        ...
        if any("compression_ratio" in r for r in successful_results):
            ...
            print(
                f"📦 Best compression: {best_ratio['compression']} ({best_ratio.get('compression_ratio', 0):.2f}x)"
            )
        ...
        if any("throughput_gb_s" in r for r in successful_results):
            ...
            print(
                f"⚡ Best throughput: {best_throughput['compression']} ({best_throughput.get('throughput_gb_s', 0):.2f} GB/s)"
            )


def main():
    parser = argparse.ArgumentParser(description="Parquet圧縮方式の性能実験")
    parser.add_argument("--table", type=str, default="lineorder", help="対象テーブル名（デフォルト: lineorder）")
    parser.add_argument("--parallel", type=int, default=16, help="並列接続数（デフォルト: 16）")
    parser.add_argument("--chunks", type=int, default=16, help="チャンク数（デフォルト: 16）")
    parser.add_argument(
        "--compressions",
        type=str,
        nargs="+",
    ...
    parser.add_argument("--output", type=str, default=None, help="結果ファイルの出力先")
    parser.add_argument("--timeout", type=int, default=300, help="各ベンチマークのタイムアウト秒数（デフォルト: 300秒）")
    parser.add_argument("--include-gzip", action="store_true", help="gzip圧縮をテストに含める（非推奨、非常に遅い）")
    ...
    if "GPUPASER_PG_DSN" not in os.environ:
        ...
        print(
            'Example: export GPUPASER_PG_DSN="dbname=postgres user=postgres host=localhost port=5432"'
        )
        return 1
