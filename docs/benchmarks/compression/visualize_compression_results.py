...
def create_comparison_table(results: list):
    ...
    if snappy_result:
        ...
        print("=" * 80)
        print(
            f"{'Compression':<12} {'Time Diff':<12} {'Size Diff':<12} {'Time Ratio':<12} {'Size Ratio'}"
        )
        print("-" * 80)
        ...
        for r in successful_results:
            ...
            print(
                f"{comp:<12} {time_diff_str:<12} {size_diff_str:<12} {time_ratio_str:<12} {size_ratio_str}"
            )
...
def main():
    parser = argparse.ArgumentParser(description="圧縮実験結果の可視化")
    parser.add_argument("json_file", type=str, help="入力JSONファイル（compression_experiment.pyの出力）")
    parser.add_argument("--output", type=str, default=None, help="グラフの保存先（指定しない場合は表示のみ）")
    parser.add_argument("--no-plot", action="store_true", help="グラフを作成せず、テーブルのみ表示")
