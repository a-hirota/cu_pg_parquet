"""
pgCuparquet - PostgreSQL to Parquet GPU-accelerated converter

PostgreSQL → Rust → GPU キューベース並列処理版
Producer-Consumerパターンで真の並列処理を実現

使用方法:
    python pgCuparquet.py [オプション]
    
    例: python pgCuparquet.py --table lineorder --parallel 16 --chunks 8
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='pgCuparquet - PostgreSQL to Parquet GPU-accelerated converter',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用例:
  # デフォルト設定で実行（lineorderテーブル、16並列、8チャンク）
  python pgCuparquet.py
  
  # 異なるテーブルで実行
  python pgCuparquet.py --table customer --parallel 8 --chunks 4
  
  # 大規模データセット向け設定
  python pgCuparquet.py --parallel 32 --chunks 16

環境変数:
  GPUPASER_PG_DSN - PostgreSQL接続文字列（必須）
    例: export GPUPASER_PG_DSN="dbname=postgres user=postgres host=localhost port=5432"
  
  RUST_PARALLEL_CONNECTIONS - Rust側の並列接続数（オプション、--parallelで上書き）
  KVIKIO_COMPAT_MODE - kvikio互換モード（オプション、GPUDirectが使えない場合に設定）
...
必要条件:
  ...
  - 十分なGPUメモリ（最低16GB推奨）
    ...
    parser.add_argument('--table', type=str, default='lineorder', 
                       help='変換対象のPostgreSQLテーブル名（デフォルト: lineorder）')
    parser.add_argument('--parallel', type=int, default=16, 
                       help='PostgreSQL並列接続数。CPUコア数に応じて調整（デフォルト: 16）')
    parser.add_argument('--chunks', type=int, default=8, 
                       help='データ分割チャンク数。GPUメモリに応じて調整（デフォルト: 8）')
