...
def main():
    ...
    try:
        print("1. メタデータ取得中...")
        columns = fetch_column_meta(conn, "SELECT * FROM lineorder LIMIT 10000")  # さらに小さく
        
        print(f"   総列数: {len(columns)}")
        
        print("2. COPY BINARY実行中...")
        copy_sql = "COPY (SELECT * FROM lineorder LIMIT 10000) TO STDOUT (FORMAT binary)"
        buf = bytearray()
    ...
    print("5. Ultimate統合版デコード（デバッグ版）...")
    print("   ステップ1: Ultimate統合バッファシステム初期化中...")
    
    try:
        # GPU Memory Manager V4のデバッグ
        from src.gpu_memory_manager_v4_ultimate import GPUMemoryManagerV4Ultimate
        gmm_v4 = GPUMemoryManagerV4Ultimate()
        print("   ステップ1.1: GPUMemoryManagerV4Ultimate作成完了")
        
        ultimate_info = gmm_v4.initialize_ultimate_buffers(columns, rows)
        print("   ステップ1.2: initialize_ultimate_buffers完了")
        
        print(f"   固定長列: {len(ultimate_info.fixed_layouts)}列")
        print(f"   可変長列: {len(ultimate_info.var_layouts)}列")
        print(f"   固定長バッファサイズ: {ultimate_info.fixed_buffer.size:,}バイト")
        print(f"   可変長バッファサイズ: {ultimate_info.var_data_buffer.size:,}バイト")
        
        print("   ステップ2: カーネル引数準備中...")
        ...
        print("   ステップ2.1: Decimal10のべき乗テーブル転送完了")
        ...
        if var_count > 0:
            ...
            if var_count > 0:
                ...
                cuda.synchronize()
            print("   ステップ2.3: 可変長列情報GPU転送完了")
        else:
            d_var_indices = cuda.to_device(np.array([], dtype=np.int32))
            d_var_offset_arrays = cuda.device_array((1, rows + 1), dtype=np.int32)
        
        # 共通NULL配列
        d_nulls_all = cuda.device_array((rows, len(columns)), dtype=np.uint8)
        print("   ステップ2.4: 共通配列準備完了")
        
        print("   ステップ3: Ultimate統合カーネル実行準備...")
        threads = 256
        blocks = (rows + threads - 1) // threads
        print(f"   カーネル構成: blocks={blocks}, threads={threads}")
        
        # カーネルインポート
        from src.cuda_kernels.arrow_gpu_pass1_ultimate_integrated import pass1_ultimate_integrated
        print("   ステップ3.1: カーネルインポート完了")
        
        print("   ステップ3.2: カーネル実行開始...")
        ...
        print("   ステップ3.3: カーネル同期待機中...")
        cuda.synchronize()
        kernel_time = time.time() - start_kernel
        print(f"   ステップ3.4: カーネル実行完了！ ({kernel_time:.4f}秒)")
        
        print("   ステップ4: 結果処理中...")
        # 実際のArrow組立は省略
        print("   ✓ Ultimate統合カーネル実行成功！")
        return True
