...
"""
SQLクエリを用いた大規模データ処理のテスト - 小チャンク最適化版
"""
...
def test_process_custom_query(sql_query: str, output_file: Optional[str] = None):
    ...
    # GPUカーネル最適化パラメータ
    os.environ['GPUPASER_MAX_THREADS'] = '256'  # スレッド数を256に制限（オーバーヘッド削減）
    os.environ['GPUPASER_MAX_BLOCKS'] = '512'   # ブロック数を512に制限（1SMあたり32ブロック程度）
...
if __name__ == "__main__":
    # 少ない行数のテスト（まず1000行で確認）
    sql_query = "SELECT * FROM lineorder LIMIT 1000"
    output_file = "test_sql_chunk_output/lineorder_small_test.parquet"
    
    # チャンクサイズの環境変数設定（非常に小さいチャンクで処理）
    os.environ['GPUPASER_MAX_CHUNK_SIZE'] = '1000'  # チャンクサイズを小さく設定
    
    success = test_process_custom_query(sql_query, output_file)
    
    if success:
        print("\n=== テスト成功: 最適化されたGPU処理が正常に完了しました ===")
    else:
        print("\n=== テスト失敗: GPU処理中にエラーが発生しました ===")
